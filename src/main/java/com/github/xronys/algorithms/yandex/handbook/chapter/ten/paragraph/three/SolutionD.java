        ...
        while (!queue.isEmpty()) {
            ...
            if (i + 1 < labyrinth.length) {
                if (!visited[i + 1][j]) {
                    if (labyrinth[i + 1][j] == '.') {
                        queue.add(new
                                Cell(i + 1, j, cell.path + "D", cell.distance + 1));
                    } else if (labyrinth[i + 1][j] == 'F') {
                        result = new Cell(i + 1, j, cell.path + "D", cell.distance + 1);
                        break;
                    } else if (labyrinth[i + 1][j] == 'K') {
                        key = new Cell(i + 1, j, cell.path + "DP", cell.distance + 1);
                    } else if (labyrinth[i + 1][j] == 'D') {
                        door = new Cell(i + 1, j, cell.path + "D", cell.distance + 1);
                    }
                }
            }
            if (i - 1 >= 0) {
                if (!visited[i - 1][j]) {
                    if (labyrinth[i - 1][j] == '.') {
                        ...
                        key = new Cell(i - 1, j, cell.path + "UP", cell.distance + 1);
                    } else if (labyrinth[i - 1][j] == 'D') {
                        door = new Cell(i - 1, j, cell.path + "U", cell.distance + 1);
                    }
                }
            }
            if (j + 1 < labyrinth[0].length) {
                if (!visited[i][j + 1]) {
                    if (labyrinth[i][j + 1] == '.') {
                        ...
                        key = new Cell(i, j + 1, cell.path + "RP", cell.distance + 1);
                    } else if (labyrinth[i][j + 1] == 'D') {
                        door = new Cell(i , j + 1, cell.path + "R", cell.distance + 1);
                    }
                }
            }
            if (j - 1 >= 0) {
                if (!visited[i][j - 1]) {
                    if (labyrinth[i][j - 1] == '.') {
                        ...
                        key = new Cell(i, j - 1, cell.path + "LP", cell.distance + 1);
                    } else if (labyrinth[i][j - 1] == 'D') {
                        door = new Cell(i, j - 1, cell.path + "R", cell.distance + 1);
                    }
                }
            ...
            if(((key != null && door != null && key.distance + door.distance <= cell.distance) || queue.isEmpty()) && !b1) {
                ...
                Cell temp = new Cell(door.i, door.j, tempPath, door.distance + key.distance + key.distance + 1);
                queue.add(temp);
            }
        }
